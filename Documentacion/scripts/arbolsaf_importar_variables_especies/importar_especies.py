"""
    Este script funciona cargando toda la informacion de los archivos .csv presentes en la carpeta dóde se encuentra el fichero
    Es importante que en la carpeta no se encuentren presentes otros archovos de extensión .csv no relacionados con el ficgero, de lo contrario el script fallará
    El script sigue los siguientes pasos:
     - Cargar todos los archivos .csv que se encuentren en la lisma cvarpeta del script
     - Analizar la informacion presenta en cada linea del fichero
       - Encontrar la especie a partir del codigo
       - Encontrar la cita a partir del codigo
       - Recorrer todas las columnas  y encontrar las variables una por una
       - Si la celda tiene un valor diferente de NA, entonces se insertara el valor de la variable para la especie en la base de datos
     - Durante todo el procesos se generan archivos .log que contiene la informacion de los pasos ejecutados
...
# se deben abrir todos los focheros csv de la carpeta actual
files_to_open = list(filter(lambda x: (x.endswith('.csv')), onlyfiles))
