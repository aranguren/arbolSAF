...
class PortafolioView(ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['variables'] = VariableTypeModel.objects.order_by('variable')
        context['referencias'] = ReferenceModel.objects.order_by('fuente_final')

        especies = SpeciesModel.objects.all()
        context['nombre_comun_values'] = [
            {"nombre_comun": e.nombre_comun, "cod_esp": e.cod_esp}
            for e in especies.order_by('nombre_comun')
        ]
        context['nombre_cientifico_values'] = [
            {"nombre_cientifico": e.nombre_cientifico, "cod_esp": e.cod_esp}
            for e in especies.order_by('nombre_cientifico')
        ]
        ...
        filtrado = ''.join([
            context['value_cod_esp'], context['value_nombre_comun'],
            context['value_nombre_cientifico'], context['value_tipo_variable'],
            context['value_referencia'], context['value_habilitada_herramienta'],
            context['value_sinonimos'],
        ])
        context['has_filters'] = len(filtrado) > 0

        for specie in context['species']:
            endemismo = False
            amenaza_iucn = None
            amenaza_nacional = None
            for var in specie.variables.all():
                ...
                if cod == 'v64':
                    ...
                    amenaza_iucn = quals[0].nombre if quals else var.valor_texto
                elif cod == 'v59':
                    quals = list(var.valores_cualitativos.all())
                    amenaza_nacional = quals[0].nombre if quals else var.valor_texto
            ...
            specie.card_primera_imagen = imgs[0] if imgs else None
    ...
    def get_queryset(self):
        ...
        tipos_de_orden = {
            'cod_esp': 'cod_esp', 'cod_esp_dec': '-cod_esp',
            'nombre_comun': 'nombre_comun', 'nombre_comun_dec': '-nombre_comun',
            'nombre_cientifico': 'nombre_cientifico', 'nombre_cientifico_dec': '-nombre_cientifico',
            'familia': 'familia__familia', 'familia_dec': '-familia__familia',
            'nativa_peru': 'nativa', 'nativa_peru_dec': '-nativa',
        ...
        qs = SpeciesModel.objects

        if query['sinonimos']:
            qs = qs.filter(sinonimos__sinonimo__icontains=query['sinonimos'])
        if query['habilitada_herramienta'] == 'habilitada':
            qs = qs.filter(habilitada_herramienta=True)
        elif query['habilitada_herramienta'] == 'desabilitada':
            qs = qs.filter(habilitada_herramienta=False)
        if query['cod_esp']:
            qs = qs.filter(cod_esp__iexact=query['cod_esp'])
        if query['nombre_comun']:
            qs = qs.filter(nombre_comun__icontains=query['nombre_comun'])
        if query['nombre_cientifico']:
            qs = qs.filter(nombre_cientifico__icontains=query['nombre_cientifico'])
        if query['tipo_variable']:
            with connection.cursor() as cursor:
                ...
                qs = qs.filter(id__in=[x[0] for x in cursor.fetchall()])
        if query['referencia']:
            with connection.cursor() as cursor:
                cursor.execute("""
                    SELECT DISTINCT as2.id FROM arbolsaf_species as2
        ...
        qs = qs.order_by(tipos_de_orden.get(orden, 'nombre_comun'))
